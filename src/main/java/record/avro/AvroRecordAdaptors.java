 ...
 */
public final class AvroRecordAdaptors {
	private AvroRecordAdaptors() {
		throw new AssertionError("should not be called: class=" + getClass());
	}
	
	public static RecordSetReader fileReader(File file) throws FileNotFoundException {
		return new AvroRecordSetFileReader(file);
	}
	
	public static RecordSetReader fileReader(byte[] bytes) {
		return new AvroRecordSetFileReader(bytes);
	}
	
	public static AvroRecordSetFileWriter fileWriter(File file) {
		return new AvroRecordSetFileWriter(file);
	}
	
	public static RecordSetWriter fileWriter(OutputStream os) {
		return new AvroRecordSetFileWriter(os);
	}
	
	public static RecordSetReader binaryReader(Schema schema, File file) throws FileNotFoundException {
		return new BinaryAvroRecordSetReader(schema, file);
	}
	
	public static RecordSetReader binaryReader(Schema schema, byte[] bytes) {
		return new BinaryAvroRecordSetReader(schema, bytes);
	}
	
	public static RecordSetReader binaryReader(Schema schema, byte[] bytes, int offset, int length) {
		return new BinaryAvroRecordSetReader(schema, bytes, offset, length);
	}
	
	public static RecordSetWriter binaryWriter(File file) throws FileNotFoundException {
		return binaryWriter(new BufferedOutputStream(new FileOutputStream(file), 4*1024));
	}
	
	public static RecordSetWriter binaryWriter(OutputStream os) {
		return new BinaryAvroRecordSetWriter(os);
	}
